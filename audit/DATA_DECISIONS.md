...
Paired-plus-binary combinations are a separate issue already visible in the raw CSV: they need no source repair, but the old loader incorrectly counted their Yea/Nay side. The new binary filter excludes any paired observation. Run `python check_data.py --output outputs/integrity.json` to list all combination-coded exclusions in `combination_votes_excluded`. Ordinary paired-only rows are excluded as before.
...
Run `python tools/build_audit_data.py --check`. It re-derives the registered JSON and reconstructed CSV from the frozen evidence, verifies all source checksums, and compares them with the saved products. The MVP analysis calls this guard before processing the corpus. A changed raw file, metadata row, derivation, or unexplained tally difference blocks analysis.

The [baseline inventory](baseline_discrepancies.json) records all 19 original problems. [data_decisions.json](data_decisions.json) records the exact operational decisions. Each MVP output manifest hashes its selected source tables and correction files. New evidence must be reviewed before changing the saved data.

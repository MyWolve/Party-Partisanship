...
**Malformed subject.** Session 42-1 division 247 names a bill but omits its number. It is now `unknown_bill`. E1 reports a separate sensitivity that assigns this division to government business. No data is discarded to conceal the uncertainty.

## Evidence and verification

The original source responses are archived under `evidence/`; [sources.json](../evidence/sources.json) maps their SHA256 fingerprints to URLs and retrieval date. The source ledger is included in the frozen-input manifest. The retrieval script is explicit and is not run during reproduction.

Tests check party scope, cabinet/backbench labels, procedural non-inheritance, unresolved stages, malformed subjects, and the curly-apostrophe bill-type regression. [E1's classification table](../experiments/results_e1/classification_audit.csv) makes each applied decision inspectable. The uncertainty-excluded comparison removes audited unresolved cases; its remaining category proxies must not be described as verified whipped votes.

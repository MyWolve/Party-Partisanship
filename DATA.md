...
| `party_votes.csv` | Division × tracked party | Division metadata plus `party`, binary `yea`, `nay` |
...
A nonempty `correction` links to a division-level entry in `data/corrections/data_decisions.json`. It flags that the division required repair, not that every member in it was changed. Raw source files remain in the original session directories. The correction JSON includes the original metadata, source paths and the reason for each change.

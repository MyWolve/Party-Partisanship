...
def xml_rows(name):
    return [{c.tag: c.text or '' for c in e}
            for e in ET.parse(ROOT / 'evidence' / name).getroot()]


def journal_names(number):
    s = (ROOT / 'evidence' / f'42-1-{number}-journals.html').read_text(encoding='utf-8')
...
def build():
    ...
    for issue in sorted(issues, key=lambda r: (r['session'], r['division'])):
        ...
        meta = next(r for r in csv.DictReader((directory / 'votes_metadata.csv').open(encoding='utf-8-sig'))
                    if int(r['vote_number']) == number)
